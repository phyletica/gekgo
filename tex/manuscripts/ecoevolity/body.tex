...
\jroedit{As a result, we}{To maximize the number of pairs and thus increase our
    chances of detecting shared divergences if they occured, we include these
...
\jroedit{}{
\subsection{Assessing our study design and methodology}
}

\jroedit{}{
\subsubsection{Simulation-based assessment of our sampling design and methods}
}
...
\jroedit{}{
    ...
    Specifically, we simulate \datasets that match our empirical dataset
    in terms of
    \begin{enumerate}
        \item the number of loci for each pair of populations,
        \item the number of sites within each locus, and
        \item the missing data patterns within each locus.
    \end{enumerate}
...
\jroedit{}{
    ...
    distributions as priors and ran 4 independent MCMC chains for 150,000
    generations, sampling every 100th generation.
    After ignoring the first 501 samples from each chain (including the initial
    state), we collected 4000 samples for each analysis.
}
...
\jroedit{}{
    To assess how well \ecoevolity would infer a true shared divergence
    given our RADseq data, we randomly split the loci from our population
    pair with the largest number of sampled loci
    (\spp{C.\ philippinicus} from Sibuyan and Tablas)
    into two subsets.
    ...
    If \ecoevolity can reliably detect a shared divergence event, it
    should infer that the two sets of loci from the same two populations
    did indeed co-diverge.
}
...
\jroedit{}{
Simulations show our data are sufficient to accurately estimate
the timing
...
They also clearly show that estimation accuracy and precision are much better
when all sites are analyzed rather than only unlinked SNPs.
}
...
\jroedit{}{
\subsubsection{Data partitioning to assess \ecoevolity}
When we reanalyzed our \spp{Cyrtodactylus} data with the
loci from the pair of \spp{C.\ philippinicus}
populations for Sibuyan and Tablas randomly split
and treated as separate comparisons,
the Bayes factors approximated by
\ecoevolity strongly supported
\citep{Jeffreys1961}
...
$\distgamma(1.1, 56.1)$,
respectively.
The prior odds were approximated using one million simulations under the
Dirichlet process.
}
