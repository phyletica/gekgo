...
A ``species-pump'' model of diversification via repeated vicariance predicts
...
structure, we only used localities where previous genetic data was
...
\citep{Welton2010, Welton2010zootaxa}.
...
populations on oceanic islands, however, these events are likely too random and
rare to contribute meaningful levels of gene flow between islands.
...
Thus, all of the pairs we sampled are independent evolutionary lineages,
regardless of whether they are currently recognized as conspecific or not.
}
...
According\text{ly}{ to all these findings}, we analyzed all of the sites of our
RADseq loci.
\jroedit{}{
...
\jroedit{}{
\subsection{Assessing our sampling and methodology}
}
...
\jroedit{}{
    ...
    and the number of loci and sites varies dramatically among our
...
\jroedit{}{
    ...
    After ignoring the first 501 samples from each chain (including the initial
    state), we collected 4000 MCMC samples for each analysis.
}

\jroedit{}{
\subsubsection{Data partitioning to assess \ecoevolity}
}
...
\jroedit{}{
    ...
    We randomly split the 21,426 loci from our population
...
\jroedit{}{
\subsection{Data partitioning to assess \ecoevolity}
}
...
For each genus we \jroedit{}{sampled} 5--6 pairs of populations that span two
...
\jroedit{}{
Nonetheless, with seven pairs (11 including ambigious island connections), we
...
island separations and the mutation rate are toward the upper end of plausable.
}
...
much larger differences than expected if divergences occurred within the same
interglacial.
...
Given the variation among island fragmentation times is small (< 5000 years)
...
the same islands can be brought to bare, this would provide a strong and
complementry test of the PAIC model.
}
...
Given the pairs within each genus are closely related and have similar life
histories, we do not expect large differences in mutation rates.
...
While ignoring migration is not ideal, it allows us to use genomic data with a
...
\jroedit{}{
While we allowed the ancestral and descendant populations to have different
...
Thus, trying to estimate complex demographic changes from the data might
introduce more error than the bias it is trying to resolve.
}


\jroedit{}{
\subsection{How does \ecoevolity behave given our RADseq data?}
}
...
Perhaps with so much genomic data, \ecoevolity fails to accurately estimate
...
\jroedit{}{
Furthermore, from analyzing 2000 \datasets that were simulated to match the
dimensions of or gekkonid RADseq data,
...
It is not too surprising that \ecoevolity performs better when using all of the
...
populations should not be affected by linked sites, as was seen here and by
...
discards a huge amount of informative data.
...
Our simulation results also show that \ecoevolity is robust to large
disparities in the number of sampled individuals and loci
(Table \ref{table:comparisons}, Figure \ref{fig:simsdivtimes} \& \ref{fig:simsnevents}).
...
For example, let's compare the estimation of divergence times between the pairs
that were simulated to match our RADseq data from \spp{C. philippinicus}
populations from the Islands of
Luzon and Babuyan Claro
versus
Polillo and Luzon.
The former pair consists of only two samples per population
and 3855 loci, whereas the latter has
