...
def get_pbs_header(walltime = "5:00:00", nthreads = 4):
    s = ("#! /bin/sh\n"
         "#PBS -q gen28\n"
         "#PBS -l nodes=1:ppn={0}\n"
         "#PBS -l walltime={1}\n"
...
def write_qsub(config_path,
    ...
    if not os.path.exists(qsub_path):
        with open(qsub_path, 'w') as out:
            out.write(get_pbs_header(walltime, nthreads = 4))
            out.write("prefix={0}\n\n".format(prefix))
            out.write(
    ...
    if not os.path.exists(no_data_qsub_path):
        with open(no_data_qsub_path, 'w') as out:
            out.write(get_pbs_header(walltime="0:10:00", nthreads = 1))
